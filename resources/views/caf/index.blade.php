                    ...
                    <div class="card-header">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <div class="float-right">
                                <a data-bs-toggle="modal" data-bs-target="#modalDownload"
                                    class="btn btn-outline-success btn-sm float-right" data-placement="left">
                                    Descargar CAF
                                </a>
                                <a href="{{ route('cafs.create') }}" class="btn btn-primary btn-sm float-right"
                                    data-placement="left">
                                    {{ __('Create New') }}
                                </a>
                            </div>
                                            ...
                                            <td>
                                                <form action="{{ route('cafs.destroy', $caf->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary "
                                                        href="{{ route('cafs.show', $caf->id) }}"><i
                                                            class="fa fa-fw fa-eye"></i> Show</a>
                                                    <a class="btn btn-sm btn-success"
                                                        href="{{ route('cafs.edit', $caf->id) }}"><i
                    ...
                    <form action="" method="post">
                        <div class="row">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <select id="ambiente" class="form-select" aria-label="Default select example" required>
        ...
        async function downloadCaf() {
            try {
                ...
                formData.append('input', JSON.stringify({
                    "RutCertificado": "18888331-1",
                    "Password": "2011Agustin",
                    "RutEmpresa": "77643522-8",
                    "Ambiente": ambiente
                }))
                formData.append('files', $file)
                var url='https://servicios.simpleapi.cl/api/folios/get/' + doc + '/' + $(
                        '#quantity_folio').val()
                // var apikey = bt('api:4406-J440-6381-4016-2182')
                fetch('https://servicios.simpleapi.cl/api/folios/get/' + doc + '/' + $(
                        '#quantity_folio').val(), {
                        method: 'POST',
                        headers: {
                            'Authorization': 'Basic ' + btoa('api:7612-R750-6383-9966-7300'),
                        },
                        dataType: 'jsonp',
                        body: formData,
                        crossDomain: true,
